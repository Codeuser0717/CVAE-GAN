...
if __name__ == '__main__':
    ...
    # 训练CGAN模型
    print("开始训练CGAN模型...")
    cgan.fit(datasets.TrDataset())
    
    # 绘制并保存损失历史曲线
    print("\n生成损失曲线...")
    cgan.plot_loss_history()
    
    # 打印原始数据集类别分布
    print("原始数据集类别分布:")
    original_class_counts = {}
    for i in cgan.samples.keys():
        original_class_counts[i] = len(cgan.samples[i])
        print(f"类别 {i}: {original_class_counts[i]} 个样本")
    
    # 计算每个类别的最大样本数
    max_cnt = max([len(cgan.samples[i]) for i in cgan.samples.keys()])
    ...
    for i in cgan.samples.keys():
        current_count = len(cgan.samples[i])
        ...
        if cnt_generated > 0:
            ...
            # 调用CGAN生成样本
            generated_samples = cgan.generate_qualified_samples(i, cnt_generated)
    ...
    # 保存增强后的数据集
    with open('data_cgan.pkl', 'wb') as f:
        pickle.dump(
            (
    ...
    utils.set_random_state()
    clf = Classifier('CGAN')
    clf.model = cgan.classifier  # 使用CGAN训练好的分类器
    ...
    with torch.no_grad():
        ...
        for label in range(datasets.label_num):
            # 生成5个样本
            generated = cgan.generate_samples(label, 5)
