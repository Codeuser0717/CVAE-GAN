...
if __name__ == '__main__':
    ...
    # 训练CVAE模型
    print("开始训练CVAE模型...")
    cvae.fit(datasets.TrDataset())
    
    # 绘制并保存损失历史曲线
    print("\n生成损失曲线...")
    cvae.plot_loss_history()
    
    # 打印原始数据集类别分布
    print("原始数据集类别分布:")
    original_class_counts = {}
    for i in cvae.samples.keys():
        original_class_counts[i] = len(cvae.samples[i])
        print(f"类别 {i}: {original_class_counts[i]} 个样本")
    
    # 计算每个类别的最大样本数
    max_cnt = max([len(cvae.samples[i]) for i in cvae.samples.keys()])
    ...
    for i in cvae.samples.keys():
        current_count = len(cvae.samples[i])
        ...
        if cnt_generated > 0:
            ...
            # 调用CVAE生成样本
            generated_samples = cvae.generate_qualified_samples(i, cnt_generated)
    ...
    # 保存增强后的数据集
    with open('data_cvae.pkl', 'wb') as f:
        pickle.dump(
            (
    ...
    utils.set_random_state()
    clf = Classifier('CVAE')
    clf.model = cvae.classifier  # 使用CVAE训练好的分类器
    ...
    with torch.no_grad():
        ...
        for label in range(datasets.label_num):
            # 生成5个样本
            generated = cvae.generate_samples(label, 5)
            ...
            if len(generated) > 0:
                ...
                # 重构一些真实样本
                real_samples = cvae._get_target_samples(label, 5)
                real_labels = torch.full([5], label, device=real_samples.device)  # 确保标签和样本在同一设备
                reconstructed = cvae.reconstruct_samples(real_samples, real_labels)
                
                # 确保real_samples和reconstructed在同一设备上
                real_samples = real_samples.to(reconstructed.device)
