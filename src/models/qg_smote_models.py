...
class QuantileEncoderModel(nn.Module):
    def __init__(self, input_dim: int, num_classes: int, latent_dim: int = 128, num_quantiles: int = 3):
        super().__init__()
        ...
        self.fc_quantiles = nn.Linear(hidden_size3, latent_dim * num_quantiles)
    ...
    def forward(self, x: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        quantiles_flat = self.fc_quantiles(hidden)
        
        quantiles = quantiles_flat.view(-1, self.latent_dim, self.num_quantiles)
        
        return quantiles
    
    def sample_from_quantiles(self, quantiles: torch.Tensor, alpha: float = None) -> torch.Tensor:
        if alpha is None:
            alpha = torch.rand(quantiles.size(0), 1, 1, device=quantiles.device)
        
        idx_float = alpha * (self.num_quantiles - 1)
        idx_lower = torch.floor(idx_float).long()
        idx_upper = torch.ceil(idx_float).long()
        weight_upper = (idx_float - idx_lower).squeeze(-1)
        weight_lower = 1 - weight_upper
        
        z_lower = torch.gather(quantiles, 2, idx_lower.expand(-1, self.latent_dim, -1)).squeeze(-1)
        z_upper = torch.gather(quantiles, 2, idx_upper.expand(-1, self.latent_dim, -1)).squeeze(-1)
        
        z = weight_lower * z_lower + weight_upper * z_upper
        
        return z


class QGGeneratorModel(nn.Module):
...
class QGDiscriminatorModel(nn.Module):
...
class QuantileRegressorModel(nn.Module):
    def __init__(self, in_features: int, num_classes: int, num_quantiles: int = 3):
        super().__init__()
        self.in_features = in_features
        self.num_classes = num_classes
        self.num_quantiles = num_quantiles
        ...
        self.regressor_network = nn.Sequential(
            nn.Linear(total_input_dim, hidden_size1),
            nn.ReLU(),
            nn.Dropout(0.3),
            ...
            nn.Linear(hidden_size2, in_features * num_quantiles),
        )
    ...
    def forward(self, x: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        output = self.regressor_network(x_cond)
        
        quantiles = output.view(-1, self.in_features, self.num_quantiles)
        
        return quantiles


class QGClassifierModel(nn.Module):
    def __init__(self, in_features: int, num_classes: int):
        super().__init__()
        self.in_features = in_features
        self.num_classes = num_classes
