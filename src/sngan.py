...
class SNGAN:
    ...
    def __init__(self):
        ...
        # 创建模型组件
        self.generator = models.SNGANGeneratorModel(
            latent_dim=config.gan_config.z_size,
            num_classes=self.label_num,
            output_dim=self.feature_num
        ).to(config.device)
        
        self.discriminator = models.SNGANDiscriminatorModel(
            in_features=self.feature_num,
            num_classes=self.label_num
        ).to(config.device)
        
        self.classifier = models.SNGANClassifierModel(
            in_features=self.feature_num,
            num_classes=self.label_num
        ).to(config.device)
        ...
        # 从配置文件加载损失权重
        self.lambda_adv = config.gan_config.sngan_config['lambda_adv']
        self.lambda_class = config.gan_config.sngan_config['lambda_class']
        ...
        for e in range(config.gan_config.epochs):
            ...
            for target_label in self.samples.keys():
                ...
                for _ in range(config.gan_config.d_loop_num):
                    ...
                    d_real_loss = torch.nn.functional.relu(1.0 - d_real).mean()
                    
                    # 生成样本损失
                    d_fake = self.discriminator(generated_samples.detach(), target_labels_tensor)
                    d_fake_loss = torch.nn.functional.relu(1.0 + d_fake).mean()
                ...
                for _ in range(config.gan_config.g_loop_num):
                    ...
                    if e < 200:  # 前200个epoch，给分类器一些时间学习
                        ...
                        progress = (e - 200) / 300
                        current_lambda_class = config.gan_config.sngan_config['lambda_class'] * progress
                    else:
                        current_lambda_class = config.gan_config.sngan_config['lambda_class']
            ...
            # 每50轮打印一次训练进度
            if e % 50 == 0:
                print(f"SNGAN训练轮次: {e}/{config.gan_config.epochs}, "
    ...
    def plot_loss_history(self):
        ...
        # 保存图片
        loss_plot_path = config.path_config.gan_outs / 'sngan_loss_history.jpg'
        ...
        # 保存综合图片
        combined_loss_path = config.path_config.gan_outs / 'sngan_combined_loss.jpg'
    ...
    def generate_qualified_samples(self, target_label: int, num: int, confidence_threshold: float = None):
        ...
        if confidence_threshold is None:
            confidence_threshold = config.gan_config.sngan_config['confidence_threshold']
